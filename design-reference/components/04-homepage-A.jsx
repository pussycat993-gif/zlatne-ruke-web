...
function Homepage() {
  ...
  return (
    <Page>
      {/* Editorial Hero */}
      <section style={{ background: 'linear-gradient(180deg, var(--zr-cream) 0%, var(--zr-cream-deep) 100%)', position: 'relative', overflow: 'hidden' }}>
        <div className="zw-container" style={{ display: 'grid', gridTemplateColumns: '1.05fr 1fr', gap: 64, padding: '80px 32px 100px', alignItems: 'center' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <div style={{ position: 'absolute', top: -16, right: -24, width: 120, height: 120, borderRadius: 999, background: 'var(--zr-surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', boxShadow: 'var(--zr-shadow)', transform: 'rotate(8deg)' }}>
      ...
      {/* Categories ribbon */}
      <section style={{ padding: '64px 0', background: 'var(--zr-surface)', borderTop: '1px solid var(--zr-border-soft)', borderBottom: '1px solid var(--zr-border-soft)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 16 }}>
            {categories.map((c, i) => (
              <a key={c.id} onClick={() => navigate('/katalog?cat=' + c.id)}
                ...
                <div style={{ width: 64, height: 64, borderRadius: 999, background: 'var(--zr-surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, marginBottom: 12, boxShadow: 'var(--zr-shadow-sm)' }}>{c.emoji}</div>
      ...
      {/* Become seller CTA */}
      <section style={{ background: 'var(--zr-hero-bg)', color: 'white', padding: '96px 0', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="zw-container" style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 80, alignItems: 'center', position: 'relative' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 36 }}>
              <button className="zw-btn zw-btn-lg" style={{ background: 'var(--zr-surface)', color: 'var(--zr-pink-dark)' }} onClick={() => navigate('/postani-prodavac')}>Otvori radnju ✨</button>
...
function ManifestPoint({ icon, t, d }) {
  return (
    <div style={{ display: 'flex', gap: 14 }}>
      <div style={{ width: 44, height: 44, borderRadius: 999, background: 'var(--zr-surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>{icon}</div>
      <div>
