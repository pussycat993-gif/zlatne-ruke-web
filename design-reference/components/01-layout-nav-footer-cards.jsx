...
// ─── Logo — Round 02 #05: ZR monogram + naziv ispod ───
function Logo({ size = 28, onClick }) {
  return (
    <div onClick={onClick || (() => navigate('/'))} style={{ display: 'inline-flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
      <span style={{ fontFamily: "'Lora', Georgia, serif", fontStyle: 'italic', fontWeight: 500, fontSize: size * 1.55, color: 'var(--zr-pink)', letterSpacing: '-0.06em', lineHeight: 0.85 }}>ZR</span>
      <span style={{ fontFamily: 'var(--zr-font), DM Sans, sans-serif', fontWeight: 600, fontSize: size * 0.4, color: 'var(--zr-pink-dark)', letterSpacing: '0.4em', textTransform: 'uppercase' }}>Zlatne · Ruke</span>
    </div>
  );
}
...
function TopNav() {
  ...
  return (
    <header style={{ background: 'var(--zr-surface)', borderBottom: '1px solid var(--zr-border-soft)', position: 'sticky', top: 0, zIndex: 50 }}>
...
// ─── Theme toggle (Svetla ↔ Tamna · zlatna) ───
function ThemeToggle() {
  const [theme, setTheme] = useState(() =>
    (typeof window !== 'undefined' && window.__zrTweaks?.theme) || 'light');

  // Keep local state in sync if Tweaks panel changes it
  useEffect(() => {
  ...
  const toggle = () => {
    ...
    setTheme(next);
    if (window.__zrSetTweak) {
      window.__zrSetTweak('theme', next);
    } else {
  ...
  return (
    <button onClick={toggle}
      title={isDark ? 'Pređi na svetlu temu' : 'Pređi na tamnu temu sa zlatnim detaljima'}
      style={{
      ...
      onMouseOver={e => { e.currentTarget.style.borderColor = 'var(--zr-pink)'; }}
      onMouseOut={e => { e.currentTarget.style.borderColor = 'var(--zr-border)'; }}>
      {/* sun/moon icon */}
      <span style={{
        width: 22, height: 22, borderRadius: 999,
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        background: isDark
          ? 'linear-gradient(135deg, #E0AC5C, #C28838)'
          : 'linear-gradient(135deg, #FBE7EC, #E8C9D2)',
        boxShadow: isDark ? '0 0 8px rgba(224, 172, 92, 0.5)' : 'none',
        fontSize: 13, lineHeight: 1,
      }}>
        {isDark ? '🌙' : '☀'}
      </span>
      <span>{isDark ? 'Tamna' : 'Svetla'}</span>
    </button>
  );
}
  ...
  return (
    <div
      style={{
        ...
        width: 720, background: 'var(--zr-surface)',
...
function Footer() {
  return (
    <footer style={{ background: 'var(--zr-hero-bg)', color: 'rgba(255,255,255,0.85)', marginTop: 80 }}>
  ...
  return (
    ...
    >
      <div onClick={() => navigate('/proizvod/' + product.id)} style={{ position: 'relative' }}>
        ...
        {product.inStock <= 3 && <div style={{ position: 'absolute', top: 12, right: 12, background: 'var(--zr-surface)', color: 'var(--zr-pink-dark)', fontSize: 10, fontWeight: 700, padding: '4px 9px', borderRadius: 999, letterSpacing: '0.05em' }}>jos {product.inStock}</div>}
      </div>
      ...
      <button
        ...
        style={{
          ...
          background: 'var(--zr-surface)', cursor: 'pointer',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 32, gap: 24 }}>
      <div>
        ...
        <h2 style={{ fontSize: 44, fontWeight: 500, lineHeight: 1.05, fontFamily: "'Lora', 'Playfair Display', Georgia, serif", fontStyle: 'italic', letterSpacing: '-0.01em' }}>{title}</h2>
