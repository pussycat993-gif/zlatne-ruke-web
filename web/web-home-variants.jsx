...
function HomepageB() {
  ...
  return (
    <Page>
      ...
      <section style={{ background: 'var(--zr-cream)', padding: '48px 0 32px', borderBottom: '1px solid var(--zr-border-soft)' }}>
        <div className="zw-container" style={{ textAlign: 'center' }}>
          ...
          <div style={{ position: 'relative', maxWidth: 620, margin: '32px auto 0' }}>
            <input placeholder="Šta tražiš? Šal, sveće, sapuni…" style={{ width: '100%', padding: '18px 24px 18px 54px', borderRadius: 999, border: '1.5px solid var(--zr-border)', background: 'white', fontSize: 15, fontFamily: 'inherit', color: 'var(--zr-pink-dark)', outline: 'none', boxShadow: 'var(--zr-shadow-sm)' }} />
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 20, flexWrap: 'wrap' }}>
            {['Šal od merino vune','Sojine sveće','Lavanda sapun','Srebrne minđuše','Domaći med'].map(t => (
              <span key={t} style={{ fontSize: 12, padding: '6px 14px', borderRadius: 999, background: 'white', color: 'var(--zr-gray)', border: '1px solid var(--zr-border-soft)', cursor: 'pointer' }}>{t}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      {/* Featured shops row */}
      <section style={{ padding: '48px 0', background: 'white', borderTop: '1px solid var(--zr-border-soft)' }}>
        <div className="zw-container">
